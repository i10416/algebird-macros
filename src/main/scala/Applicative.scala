...
/** Simple implementation of an Applicative type-class. There are many choices
  * for the canonical second operation (join, sequence, joinWith, ap), all
  * equivalent. For a Functor modeling concurrent computations with failure,
  * like Future, combining results with join can save a lot of time over
  * combining with flatMap. (Given two operations, if the second fails before
  * the first completes, one can fail the entire computation right then. With
  * flatMap, one would have to wait for the first operation to complete before
  * failing it.)
  *
  * Laws Applicatives must follow: map(apply(x))(f) == apply(f(x))
  * join(apply(x), apply(y)) == apply((x, y)) (sequence and joinWith specialize
  * join - they should behave appropriately)
  */
...
object Applicative {
  ...
  def join[M[_], T, U](mt: M[T], mu: M[U])(implicit
      app: Applicative[M]
  ): M[(T, U)] =
    app.join(mt, mu)
  def join[M[_], T1, T2, T3](m1: M[T1], m2: M[T2], m3: M[T3])(implicit
      app: Applicative[M]
  ): M[(T1, T2, T3)] =
    app.join(m1, m2, m3)
  ...
  def sequence[M[_], T](ms: Seq[M[T]])(implicit
      app: Applicative[M]
  ): M[Seq[T]] =
    app.sequence(ms)
  ...
  def sequenceGen[M[_], T, S[X] <: TraversableOnce[X], R[_]](
      ...
      app: Applicative[M],
      cbf: scala.collection.Factory[T, R[T]]
  ): M[R[T]] = {
  ...
  def joinWith[M[_], T, U, V](mt: M[T], mu: M[U])(fn: (T, U) => V)(implicit
      app: Applicative[M]
  ): M[V] =
  ...
  implicit def operators[A, M[_]](m: M[A])(implicit
      app: Applicative[M]
  ): ApplicativeOperators[A, M] =
...
/** This enrichment allows us to use our Applicative instances in for
  * expressions: if (import Applicative._) has been done
  */
class ApplicativeOperators[A, M[_]](m: M[A])(implicit app: Applicative[M])
    extends FunctorOperators[A, M](m) {
...
/** This is a Semigroup, for all Applicatives.
  */
class ApplicativeSemigroup[T, M[_]](implicit
    ap: Applicative[M],
    sg: Semigroup[T]
...
/** Group and Ring ARE NOT AUTOMATIC. You have to check that the laws hold for
  * your Applicative. If your M[_] is a wrapper type (Option[_], Some[_],
  * Try[_], Future[_], etc...) this generally works.
  */
